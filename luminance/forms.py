...
class ContestForm(Form):
    ...
    start_date = DateTimeField(
        'start_date', format="%Y-%m-%dT%H:%M", validators=[validators.Optional()])
    end_date = DateTimeField(
        'end_date', format="%Y-%m-%dT%H:%M", validators=[validators.Optional()])
    max_registrants = IntegerField(
        'max_registrants', validators=[validators.Optional()])
